...
const LatestAdditions = async () => {
  ...
  return (
    <div className="w-full px-4 py-12 bg-[#FFEED6] flex flex-col items-center justify-center relative ">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 z-10 w-full max-w-6xl">
        {books.map((book: Book) => (
          <div
            key={book._id}
            className="bg-[#DE3D3A] rounded-lg p-4 flex flex-col items-center shadow-lg"
          >
            ...
            <Link href={`/books/${book._id}`}>
              <button className="bg-white text-[#DE3D3A] px-4 py-2 rounded-lg">
                View Details
              </button>
            </Link>
          </div>
